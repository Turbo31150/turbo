...
- git_status: Git status du projet courant | triggers: [git status, statut git, etat du repo] | action: powershell:cd /home/turbo/jarvis-m1-ops; git status
- git_log: Git log recent | triggers: [git log, historique git, derniers commits] | action: powershell:cd /home/turbo/jarvis-m1-ops; git log --oneline -10
- git_pull: Git pull origin main | triggers: [git pull, tire les changements, pull git] | action: powershell:cd /home/turbo/jarvis-m1-ops; git pull origin main
- git_push: Git push origin main | triggers: [git push, pousse les commits, push git] | action: powershell:cd /home/turbo/jarvis-m1-ops; git push origin main
- pip_list: Lister les packages Python installes | triggers: [pip list, packages python, quels packages] | action: powershell:& '/home/turbo\.local\bin\uv.exe' run --directory /home/turbo/jarvis-m1-ops pip list | Out-String
- python_version: Version Python et uv | triggers: [version python, quelle version python, python version] | action: powershell:& '/home/turbo\.local\bin\uv.exe' run --directory /home/turbo/jarvis-m1-ops python --version; & '/home/turbo\.local\bin\uv.exe' --version
...
- ouvrir_projets: Ouvrir le dossier projets | triggers: [ouvre mes projets, va dans les projets, ouvre le dossier turbo] | action: powershell:Start-Process explorer.exe -ArgumentList '/home/turbo/jarvis-m1-ops'
...
## JARVIS (12 commandes)
- historique_commandes: Voir l'historique des commandes JARVIS | triggers: [historique des commandes, quelles commandes j'ai utilise, dernieres commandes] | action: powershell:if (Test-Path '/home/turbo/jarvis-m1-ops\data\action_history.json') { Get-Content '/home/turbo/jarvis-m1-ops\data\action_history.json' | ConvertFrom-Json | Select -Last 10 | Out-String } else { 'Aucun historique' }
