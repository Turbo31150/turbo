...
COMMANDS: list[JarvisCommand] = [
    ...
    # ── Batch 108 — ORM/Serverless/Testing/Temporel (15 commandes) ──
    JarvisCommand("prisma_status", "dev", "Prisma status", [
        "prisma status", "check prisma", "prisma info",
        "prisma migrate", "prisma studio",
    ], "bash", "npx prisma --version 2>/dev/null || echo 'Prisma N/A'"),
    JarvisCommand("sqlalchemy_check", "dev", "Check SQLAlchemy", [
        "check sqlalchemy", "sqlalchemy info", "orm check",
        "check orm", "sqlalchemy version",
    ], "bash", "python -c \"import sqlalchemy;print(f'SQLAlchemy {sqlalchemy.__version__}')\" 2>/dev/null || echo 'SQLAlchemy N/A'"),
    JarvisCommand("mongoose_check", "dev", "Check Mongoose", [
        "check mongoose", "mongoose status", "mongo orm",
        "mongoose info", "mongoose version",
    ], "bash", "npm list mongoose 2>/dev/null | head -3 || echo 'Mongoose N/A'"),
    JarvisCommand("run_migrations", "dev", "Run migrations", [
        "run migrations", "migrate", "execute migrations",
        "lance les migrations", "migration run",
    ], "bash", "echo 'Migrations: detect framework first' && ls **/migrations/ 2>/dev/null | head -5 || echo 'No migrations dir found'"),
    JarvisCommand("lambda_check", "dev", "Check Lambda", [
        "check lambda", "lambda status", "aws lambda",
        "lambda info", "liste les lambdas",
    ], "bash", "aws lambda list-functions --max-items 5 2>/dev/null || echo 'AWS Lambda N/A'"),
    JarvisCommand("vercel_deploy", "dev", "Vercel deploy", [
        "vercel deploy", "deploie vercel", "vercel status",
        "vercel info", "check vercel",
    ], "bash", "vercel --version 2>/dev/null || echo 'Vercel N/A'"),
    JarvisCommand("netlify_deploy", "dev", "Netlify deploy", [
        "netlify deploy", "deploie netlify", "netlify status",
        "check netlify", "netlify info",
    ], "bash", "netlify --version 2>/dev/null || echo 'Netlify N/A'"),
    JarvisCommand("jest_run", "dev", "Run Jest tests", [
        "lance jest", "jest run", "run jest",
        "test jest", "jest test",
    ], "bash", "npx jest --version 2>/dev/null || echo 'Jest N/A'"),
    JarvisCommand("cypress_run", "dev", "Run Cypress", [
        "lance cypress", "cypress run", "cypress test",
        "test cypress", "cypress e2e",
    ], "bash", "npx cypress --version 2>/dev/null || echo 'Cypress N/A'"),
    JarvisCommand("playwright_run", "dev", "Run Playwright", [
        "lance playwright", "playwright test", "run playwright",
        "test playwright", "playwright run",
    ], "bash", "npx playwright --version 2>/dev/null || echo 'Playwright N/A'"),
    JarvisCommand("selenium_check", "dev", "Check Selenium", [
        "check selenium", "selenium status", "lance selenium",
        "selenium info", "selenium version",
    ], "bash", "python -c \"import selenium;print(f'Selenium {selenium.__version__}')\" 2>/dev/null || echo 'Selenium N/A'"),
    JarvisCommand("e2e_tests", "dev", "Tests end to end", [
        "tests end to end", "tests e2e", "e2e run",
        "lance les tests e2e", "end to end",
    ], "bash", "echo '=== E2E Tests ===' && npx playwright test 2>/dev/null || npx cypress run 2>/dev/null || echo 'No E2E framework found'"),
    JarvisCommand("integration_tests", "dev", "Tests integration", [
        "tests integration", "integration test", "test integration",
        "lance les tests integration", "integration run",
    ], "bash", "pytest tests/integration/ 2>/dev/null || npm test -- --testPathPattern=integration 2>/dev/null || echo 'No integration tests found'"),
    JarvisCommand("sequelize_check", "dev", "Check Sequelize", [
        "check sequelize", "sequelize info", "sequelize status",
        "sequelize version", "sequelize migrate",
    ], "bash", "npx sequelize --version 2>/dev/null || echo 'Sequelize N/A'"),
    JarvisCommand("serverless_deploy", "dev", "Serverless deploy", [
        "deploy serverless", "serverless deploy", "sls deploy",
        "serverless info", "check serverless",
    ], "bash", "serverless --version 2>/dev/null || npx sls --version 2>/dev/null || echo 'Serverless N/A'"),
    # ── Batch 109 — StateMgmt/Gateway/CI-CD/Onomatopees (15 commandes) ──
    JarvisCommand("redux_check", "dev", "Check Redux", [
        "check redux", "redux status", "redux store",
        "check redux store", "redux info",
    ], "bash", "npm list redux 2>/dev/null | head -3 || echo 'Redux N/A'"),
    JarvisCommand("zustand_check", "dev", "Check Zustand", [
        "check zustand", "zustand status", "zustand store",
        "zustand info", "check zustand store",
    ], "bash", "npm list zustand 2>/dev/null | head -3 || echo 'Zustand N/A'"),
    JarvisCommand("nginx_check", "systeme", "Check Nginx", [
        "check nginx", "nginx status", "nginx info",
        "nginx config", "nginx test",
    ], "bash", "nginx -v 2>&1 || echo 'Nginx N/A'"),
    JarvisCommand("kong_check", "systeme", "Check Kong", [
        "check kong", "kong status", "kong info",
        "kong gateway", "kong routes",
    ], "bash", "kong version 2>/dev/null || echo 'Kong N/A'"),
    JarvisCommand("traefik_check", "systeme", "Check Traefik", [
        "check traefik", "traefik status", "traefik info",
        "traefik proxy", "traefik dashboard",
    ], "bash", "traefik version 2>/dev/null || echo 'Traefik N/A'"),
    JarvisCommand("haproxy_check", "systeme", "Check HAProxy", [
        "check haproxy", "haproxy status", "haproxy info",
        "haproxy config", "haproxy stats",
    ], "bash", "haproxy -v 2>/dev/null | head -2 || echo 'HAProxy N/A'"),
    JarvisCommand("github_actions", "dev", "GitHub Actions", [
        "github actions", "check github actions", "gh actions",
        "actions status", "ci github",
    ], "bash", "gh run list --limit 5 2>/dev/null || echo 'gh CLI N/A'"),
    JarvisCommand("gitlab_ci", "dev", "GitLab CI", [
        "gitlab ci", "check gitlab ci", "gitlab pipeline",
        "ci gitlab", "gitlab jobs",
    ], "bash", "cat .gitlab-ci.yml 2>/dev/null | head -20 || echo 'No .gitlab-ci.yml'"),
    JarvisCommand("jenkins_check", "dev", "Check Jenkins", [
        "check jenkins", "jenkins status", "jenkins info",
        "jenkins build", "jenkins pipeline",
    ], "bash", "curl -s http://127.0.0.1:8080/api/json 2>/dev/null | head -5 || echo 'Jenkins N/A'"),
    JarvisCommand("argocd_check", "dev", "Check ArgoCD", [
        "check argocd", "argocd status", "argocd apps",
        "argo cd", "argocd info",
    ], "bash", "argocd version 2>/dev/null || echo 'ArgoCD N/A'"),
    JarvisCommand("caddy_check", "systeme", "Check Caddy", [
        "check caddy", "caddy status", "caddy info",
        "caddy server", "caddy version",
    ], "bash", "caddy version 2>/dev/null || echo 'Caddy N/A'"),
    JarvisCommand("api_gateway_check", "dev", "Check API gateway", [
        "check api gateway", "api gateway status", "gateway check",
        "api gateway info", "gateway status",
    ], "bash", "echo '=== API Gateway ===' && nginx -v 2>&1 || echo 'Nginx N/A' && traefik version 2>/dev/null || echo 'Traefik N/A'"),
    JarvisCommand("ci_pipeline_status", "dev", "CI pipeline status", [
        "check ci pipeline", "ci status", "pipeline ci",
        "ci pipeline", "build status",
    ], "bash", "gh run list --limit 5 2>/dev/null || cat .github/workflows/*.yml 2>/dev/null | head -20 || echo 'No CI found'"),
    JarvisCommand("pinia_check", "dev", "Check Pinia", [
        "check pinia", "pinia store", "pinia status",
        "check pinia store", "pinia info",
    ], "bash", "npm list pinia 2>/dev/null | head -3 || echo 'Pinia N/A'"),
    JarvisCommand("vuex_check", "dev", "Check Vuex", [
        "check vuex", "vuex store", "vuex status",
        "check vuex store", "vuex info",
    ], "bash", "npm list vuex 2>/dev/null | head -3 || echo 'Vuex N/A'"),
    # ── Batch 110 — PackageMgr/Docs/Storage/Quantificateurs (15 commandes) ──
    JarvisCommand("pip_list", "dev", "Pip list packages", [
        "pip list", "liste pip", "packages pip",
        "pip packages", "pip installed",
    ], "bash", "pip list 2>/dev/null | tail -20 || echo 'pip N/A'"),
    JarvisCommand("pnpm_check", "dev", "Check pnpm", [
        "check pnpm", "pnpm status", "pnpm info",
        "pnpm version", "pnpm install",
    ], "bash", "pnpm --version 2>/dev/null || echo 'pnpm N/A'"),
    JarvisCommand("yarn_check", "dev", "Check yarn", [
        "check yarn", "yarn status", "yarn version",
        "yarn info", "yarn install",
    ], "bash", "yarn --version 2>/dev/null || echo 'yarn N/A'"),
    JarvisCommand("cargo_check", "dev", "Check cargo", [
        "check cargo", "cargo build", "cargo status",
        "cargo version", "rust cargo",
    ], "bash", "cargo --version 2>/dev/null || echo 'cargo N/A'"),
    JarvisCommand("conda_check", "dev", "Check conda", [
        "check conda", "conda info", "conda status",
        "conda version", "conda envs",
    ], "bash", "conda info 2>/dev/null | head -10 || echo 'conda N/A'"),
    JarvisCommand("poetry_check", "dev", "Check poetry", [
        "check poetry", "poetry status", "poetry info",
        "poetry version", "poetry install",
    ], "bash", "poetry --version 2>/dev/null || echo 'poetry N/A'"),
    JarvisCommand("swagger_open", "dev", "Ouvre Swagger", [
        "ouvre swagger", "swagger ui", "swagger open",
        "swagger docs", "api docs",
    ], "bash", "echo 'Swagger UI: check localhost:PORT/docs or /swagger' && curl -s http://127.0.0.1:8000/docs 2>/dev/null | head -5 || echo 'No Swagger at :8000'"),
    JarvisCommand("sphinx_build", "dev", "Build Sphinx docs", [
        "sphinx build", "lance sphinx", "build docs sphinx",
        "sphinx docs", "genere sphinx",
    ], "bash", "sphinx-build --version 2>/dev/null || echo 'Sphinx N/A'"),
    JarvisCommand("mkdocs_serve", "dev", "MkDocs serve", [
        "mkdocs serve", "lance mkdocs", "mkdocs dev",
        "mkdocs start", "docs mkdocs",
    ], "bash", "mkdocs --version 2>/dev/null || echo 'MkDocs N/A'"),
    JarvisCommand("s3_list", "systeme", "Liste S3 buckets", [
        "liste s3", "s3 buckets", "check s3",
        "aws s3", "s3 list",
    ], "bash", "aws s3 ls 2>/dev/null | head -10 || echo 'AWS S3 N/A'"),
    JarvisCommand("minio_check", "systeme", "Check MinIO", [
        "check minio", "minio status", "minio info",
        "minio server", "test minio",
    ], "bash", "mc admin info 2>/dev/null | head -5 || echo 'MinIO N/A'"),
    JarvisCommand("brew_check", "dev", "Check brew", [
        "check brew", "brew status", "brew info",
        "brew version", "homebrew",
    ], "bash", "brew --version 2>/dev/null || echo 'Homebrew N/A'"),
    JarvisCommand("npm_outdated", "dev", "npm outdated", [
        "npm outdated", "packages obsoletes", "npm update check",
        "packages a jour", "check npm updates",
    ], "bash", "npm outdated 2>/dev/null | head -15 || echo 'npm N/A'"),
    JarvisCommand("upload_file", "systeme", "Upload fichier", [
        "upload fichier", "upload file", "televerse",
        "envoie le fichier", "upload",
    ], "bash", "echo 'Upload: specifiez destination (S3/MinIO/Azure) et fichier'"),
    JarvisCommand("doc_generate", "dev", "Genere documentation", [
...
VOICE_CORRECTIONS: dict[str, str] = {
    ...
    # Vague 149 — Database ORM
    "sequel aise": "SQLAlchemy",
    "esse cul alchimie": "SQLAlchemy",
    "sql alchemy": "SQLAlchemy",
    "priis ma": "Prisma",
    "prisme a": "Prisma",
    "type orm": "TypeORM",
    "taille porme": "TypeORM",
    "django orm": "Django ORM",
    "jango orm": "Django ORM",
    "tortoise orm": "Tortoise ORM",
    "tortue orme": "Tortoise ORM",
    "sequelize": "Sequelize",
    "sequel ize": "Sequelize",
    ...
    # Vague 150 — Serverless / FaaS
    "lambda function": "Lambda function",
    "la m'bda": "Lambda",
    "lam beda": "Lambda",
    "cloud functions": "Cloud Functions",
    "cloud fonctions": "Cloud Functions",
    "azure functions": "Azure Functions",
    "a jure fonctions": "Azure Functions",
    "vercel edge": "Vercel Edge",
    "ver sel": "Vercel",
    "netlify functions": "Netlify Functions",
    "nette li faille": "Netlify",
    "edge functions": "Edge Functions",
    "edge fonctions": "Edge Functions",
    "fargate": "Fargate",
    "far gate": "Fargate",
    "cloud run": "Cloud Run",
    "cloud rune": "Cloud Run",
    "knative": "Knative",
    "ka native": "Knative",
    # Vague 151 — Testing frameworks
    "jest test": "Jest test",
    "jeste": "Jest",
    "mocha test": "Mocha test",
    "moka teste": "Mocha",
    "cypress test": "Cypress test",
    "saille press": "Cypress",
    "playwright test": "Playwright test",
    "play write": "Playwright",
    "selenium": "Selenium",
    "sel en yum": "Selenium",
    "pytest fixture": "pytest fixture",
    "pie teste fixe ture": "pytest fixture",
    "robot framework": "Robot Framework",
    "robot frame work": "Robot Framework",
    "cucumber test": "Cucumber test",
    "concombre": "Cucumber",
    "test cafe": "TestCafe",
    "teste cafe": "TestCafe",
    # Vague 152 — Expressions temporelles avancees
    "tout a l'heure": "plus tard",
    "dans deux secondes": "bientot",
    "quand t'as le temps": "pas urgent",
    "des que possible": "des que possible",
    "en priorite": "priorite haute",
    "apres ca": "ensuite",
    "avant ca": "d'abord",
    "pour demain": "pour demain",
    "c'est pour hier": "c'est urgent",
    "la semaine pro": "la semaine prochaine",
    "le mois prochain": "le mois prochain",
    "quand c'est pret": "quand c'est pret",
    "pas de rush": "pas urgent",
    "take your time": "pas urgent",
    "prends ton temps": "pas urgent",
    "when ready": "quand c'est pret",
    "a la fin": "en dernier",
    "en premier": "en premier",
    # Vague 153 — State management
    ...
    "contexte api": "Context API",
    "signal": "signals",
    "signaux": "signals",
    # Vague 154 — API gateway / proxy
    ...
    "raille te limit": "rate limiting",
    # Vague 155 — CI/CD avance
    "github actions": "GitHub Actions",
    "git hub actions": "GitHub Actions",
    "gitlab ci": "GitLab CI",
    "gite lab si aille": "GitLab CI",
    "circle ci": "CircleCI",
    "cercle ci aille": "CircleCI",
    ...
    "ar go ce des": "ArgoCD",
    "drone ci": "Drone CI",
    "drone si aille": "Drone CI",
    "buildkite": "Buildkite",
    "bilde kite": "Buildkite",
    "concourse ci": "Concourse CI",
    "con course": "Concourse",
    "harness ci": "Harness CI",
    "arness": "Harness",
    # Vague 156 — Onomatopees / sons vocaux
    "bip": "bip",
    "ding": "ding",
    "tuuut": "beep",
    "ping": "ping",
    ...
    "po aitri": "poetry",
    # Vague 158 — Documentation tools
    "sphinx doc": "Sphinx docs",
    "sfinks": "Sphinx",
    "mkdocs serve": "MkDocs serve",
    "eme ka docs": "MkDocs",
    "docusaurus": "Docusaurus",
    "doc u saure us": "Docusaurus",
    ...
    "readme": "README",
    "raide mi": "README",
    # Vague 159 — Cloud storage
    "s3 bucket": "S3 bucket",
    "esse trois": "S3",
    "azure blob": "Azure Blob",
    "a jure blob": "Azure Blob",
    "gcs bucket": "GCS bucket",
    "ji ce esse": "GCS",
    "cloudflare r2": "Cloudflare R2",
    "cloud flaire": "Cloudflare",
    "minio": "MinIO",
    "mi ni o": "MinIO",
    "backblaze": "Backblaze B2",
    "baque blaze": "Backblaze",
    "digital ocean spaces": "DigitalOcean Spaces",
    "digi tal o cean": "DigitalOcean",
    "wasabi storage": "Wasabi",
    "oua za bi": "Wasabi",
    # Vague 160 — Quantificateurs / nombres vocaux
    "une dizaine": "environ 10",
    "une vingtaine": "environ 20",
    "une trentaine": "environ 30",
    "une cinquantaine": "environ 50",
    "une centaine": "environ 100",
    "un millier": "environ 1000",
    "quelques": "quelques",
