...
def build_light_app(tool_whitelist: set[str] | None = None) -> Server:
    ...
    WRITE_TOOLS = {"write_text_file", "lm_load_model", "browser_click"}
    ...
    async def call_tool(name: str, arguments: dict) -> list[TextContent]:
        ...
        # Guardrail: block dangerous tools unless confirm=true
        if name in DANGEROUS_TOOLS and not arguments.pop("confirm", False):
            return _error(
                f"SECURITE: {name} est un outil dangereux ({DANGEROUS_TOOLS[name]}). "
                f"Ajoutez 'confirm': true dans les arguments pour confirmer l'execution."
            )

        # Guardrail: log write-capable tools
        if name in WRITE_TOOLS or name in DANGEROUS_TOOLS:
            logger.warning("GUARDRAIL: write/dangerous tool called: %s args=%s", name, str(arguments)[:200])
