...
async def wire_all() -> dict[str, int]:
    ...
    async def on_drift_detected(data: dict[str, Any]) -> None:
        ...
        try:
            ...
            await _notify(f"?? Drift qualité détecté sur {model} — reroutage automatique", "warning")
    ...
    async def on_node_offline(data: dict[str, Any]) -> None:
        ...
        try:
            ...
            await _notify(f"?? Nœud {node} hors ligne — tentative de récupération", "critical")
    ...
    async def on_node_online(data: dict[str, Any]) -> None:
        ...
        await _notify(f"?? Nœud {node} de retour en ligne", "info")
    ...
    async def on_gpu_overload(data: dict[str, Any]) -> None:
        ...
        try:
            ...
            await _notify(f"??? GPU surchargé: {temp}°C, VRAM {vram_pct}%", "warning")
    ...
    async def on_trading_risk(data: dict[str, Any]) -> None:
        ...
        try:
            msg = data.get("message", "Risque trading détecté")
    ...
    async def on_pattern_detected(data: dict[str, Any]) -> None:
        ...
        try:
            ...
            if confidence >= 0.8:
                ...
                await _notify(
                    f"?? Nouveau pattern détecté (conf={confidence:.0%}): {pattern}", "info"
                )
    ...
    async def on_task_failed(data: dict[str, Any]) -> None:
        ...
        try:
            ...
            if fail_count >= 3:
                await _notify(
                    f"?? Tâche autonome '{task}' en échec x{fail_count}: {error}", "critical"
                )
            elif fail_count >= 2:
    ...
    async def on_security_alert(data: dict[str, Any]) -> None:
        ...
        try:
            msg = data.get("message", "Alerte sécurité")
    ...
    async def on_budget_warning(data: dict[str, Any]) -> None:
        ...
        try:
            ...
            await _notify(
                f"?? Budget tokens: {used_pct:.0f}% utilisé, {remaining} restants", "warning"
            )
        except Exception as e:
