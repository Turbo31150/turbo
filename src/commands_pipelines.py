...
PIPELINE_COMMANDS: list[JarvisCommand] = [
    ...
    ], "pipeline", "browser:navigate:http://127.0.0.1:8080;;sleep:1;;powershell:nvidia-smi --query-gpu=name,temperature.gpu,utilization.gpu,memory.used,memory.total --format=csv,noheader;;powershell:$m2 = try{(Invoke-WebRequest -Uri 'http://192.168.1.26:1234/api/v1/models' -Headers @{'Authorization'='Bearer LMSTUDIO_KEY_M2_REDACTED'} -TimeoutSec 3 -UseBasicParsing).StatusCode}catch{0}; $ol1 = try{(Invoke-WebRequest -Uri 'http://127.0.0.1:11434/api/tags' -TimeoutSec 3 -UseBasicParsing).StatusCode}catch{0}; \"M2: $(if($m2 -eq 200){'OK'}else{'OFFLINE'}) | OL1: $(if($ol1 -eq 200){'OK'}else{'OFFLINE'})\""),
    ...
    ], "pipeline", "powershell:powercfg /setactive 8c5e7fda-e8bf-4a96-9a85-a6e23a8c635c;;app_open:code;;sleep:1;;app_open:wt;;sleep:1;;powershell:$m2 = try{(Invoke-WebRequest -Uri 'http://192.168.1.26:1234/api/v1/models' -Headers @{'Authorization'='Bearer LMSTUDIO_KEY_M2_REDACTED'} -TimeoutSec 3 -UseBasicParsing).StatusCode}catch{0}; \"Cluster M2: $(if($m2 -eq 200){'OK'}else{'OFF'}) — pret a bosser\""),
    ...
    ], "pipeline", "app_open:code;;sleep:2;;hotkey:win+left;;sleep:1;;browser:navigate:https://www.tradingview.com;;sleep:1;;hotkey:win+right;;sleep:1;;powershell:$m2 = try{(Invoke-WebRequest -Uri 'http://192.168.1.26:1234/api/v1/models' -Headers @{'Authorization'='Bearer LMSTUDIO_KEY_M2_REDACTED'} -TimeoutSec 3 -UseBasicParsing).StatusCode}catch{0}; \"Cluster M2: $(if($m2 -eq 200){'OK'}else{'OFF'})\""),
    ...
    ], "pipeline", "powershell:try{$r=Invoke-WebRequest http://127.0.0.1:11434/api/tags -UseBasicParsing -TimeoutSec 3; \"OL1: OK ($($r.StatusCode))\"}catch{\"OL1: OFFLINE\"};;powershell:try{$r=Invoke-WebRequest http://192.168.1.26:1234/api/v1/models -Headers @{Authorization='Bearer LMSTUDIO_KEY_M2_REDACTED'} -UseBasicParsing -TimeoutSec 3; \"M2: OK ($($r.StatusCode))\"}catch{\"M2: OFFLINE\"};;powershell:try{$r=Invoke-WebRequest http://192.168.1.113:1234/api/v1/models -UseBasicParsing -TimeoutSec 3; \"M3: OK ($($r.StatusCode))\"}catch{\"M3: OFFLINE\"};;powershell:try{$r=Invoke-WebRequest http://10.5.0.2:1234/api/v1/models -Headers @{Authorization='Bearer LMSTUDIO_KEY_M1_REDACTED'} -UseBasicParsing -TimeoutSec 3; \"M1: OK ($($r.StatusCode))\"}catch{\"M1: OFFLINE\"};;powershell:\"Endpoints check termine\""),
...
JarvisCommand("sim_db_backup_all", "pipeline", "Backup toutes les DBs: jarvis + etoile + trading", [
    ...
    ], "pipeline", "powershell:$sw=[Diagnostics.Stopwatch]::StartNew(); try{Invoke-WebRequest http://127.0.0.1:11434/api/tags -UseBasicParsing -TimeoutSec 3 >$null; $sw.Stop(); \"OL1: $($sw.ElapsedMilliseconds)ms\"}catch{\"OL1: OFFLINE\"};;powershell:$sw=[Diagnostics.Stopwatch]::StartNew(); try{Invoke-WebRequest http://192.168.1.26:1234/api/v1/models -Headers @{Authorization='Bearer LMSTUDIO_KEY_M2_REDACTED'} -UseBasicParsing -TimeoutSec 3 >$null; $sw.Stop(); \"M2: $($sw.ElapsedMilliseconds)ms\"}catch{\"M2: OFFLINE\"};;powershell:$sw=[Diagnostics.Stopwatch]::StartNew(); try{Invoke-WebRequest http://192.168.1.113:1234/api/v1/models -UseBasicParsing -TimeoutSec 3 >$null; $sw.Stop(); \"M3: $($sw.ElapsedMilliseconds)ms\"}catch{\"M3: OFFLINE\"};;powershell:\"Benchmark cluster termine\""),
    ...
    ], "pipeline", "powershell:$m1 = try{(Invoke-WebRequest -Uri 'http://10.5.0.2:1234/api/v1/models' -Headers @{'Authorization'='Bearer LMSTUDIO_KEY_M1_REDACTED'} -TimeoutSec 3 -UseBasicParsing).StatusCode}catch{0}; $m2 = try{(Invoke-WebRequest -Uri 'http://192.168.1.26:1234/api/v1/models' -Headers @{'Authorization'='Bearer LMSTUDIO_KEY_M2_REDACTED'} -TimeoutSec 3 -UseBasicParsing).StatusCode}catch{0}; $m3 = try{(Invoke-WebRequest -Uri 'http://192.168.1.113:1234/api/v1/models' -Headers @{'Authorization'='Bearer LMSTUDIO_KEY_M3_REDACTED'} -TimeoutSec 3 -UseBasicParsing).StatusCode}catch{0}; $ol1 = try{(Invoke-WebRequest -Uri 'http://127.0.0.1:11434/api/tags' -TimeoutSec 3 -UseBasicParsing).StatusCode}catch{0}; \"Cluster: M1=$(if($m1 -eq 200){'OK'}else{'OFF'}) | M2=$(if($m2 -eq 200){'OK'}else{'OFF'}) | M3=$(if($m3 -eq 200){'OK'}else{'OFF'}) | OL1=$(if($ol1 -eq 200){'OK'}else{'OFF'})\";;powershell:nvidia-smi --query-gpu=name,temperature.gpu,memory.used,memory.total --format=csv,noheader,nounits 2>&1 | Out-String"),
    ...
    ], "pipeline", "powershell:$r = Invoke-RestMethod -Uri 'http://10.5.0.2:1234/api/v1/models' -Headers @{'Authorization'='Bearer LMSTUDIO_KEY_M1_REDACTED'} -TimeoutSec 5; $loaded = ($r.models | Where-Object {$_.loaded_instances.Count -gt 0}).key -join ', '; \"M1: $loaded\";;powershell:$r = Invoke-RestMethod -Uri 'http://192.168.1.26:1234/api/v1/models' -Headers @{'Authorization'='Bearer LMSTUDIO_KEY_M2_REDACTED'} -TimeoutSec 5; $loaded = ($r.models | Where-Object {$_.loaded_instances.Count -gt 0}).key -join ', '; \"M2: $loaded\";;powershell:$r = Invoke-RestMethod -Uri 'http://192.168.1.113:1234/api/v1/models' -Headers @{'Authorization'='Bearer LMSTUDIO_KEY_M3_REDACTED'} -TimeoutSec 5; $loaded = ($r.models | Where-Object {$_.loaded_instances.Count -gt 0}).key -join ', '; \"M3: $loaded\";;powershell:$r = Invoke-RestMethod -Uri 'http://127.0.0.1:11434/api/tags' -TimeoutSec 5; \"OL1: \" + ($r.models.name -join ', ')"),
    ...
    ], "pipeline", "powershell:Invoke-RestMethod -Uri 'http://10.5.0.2:1234/api/v1/models/load' -Method Post -Body '{\"model\":\"qwen3-8b\"}' -ContentType 'application/json' -Headers @{'Authorization'='Bearer LMSTUDIO_KEY_M1_REDACTED'} -TimeoutSec 60; 'M1 qwen3-8b rechargement lance';;sleep:5;;powershell:$r = Invoke-RestMethod -Uri 'http://10.5.0.2:1234/api/v1/models' -Headers @{'Authorization'='Bearer LMSTUDIO_KEY_M1_REDACTED'} -TimeoutSec 5; $loaded = ($r.models | Where-Object {$_.loaded_instances.Count -gt 0}).key -join ', '; \"M1 modeles actifs: $loaded\"", confirm=True),
    ...
    JarvisCommand("debug_latence_cluster", "pipeline", "Mesurer la latence vers chaque noeud du cluster IA", [
        ...
        "test latence ia",
    ], "pipeline", "powershell:$nodes = @(@{Name='M1';Url='http://10.5.0.2:1234/api/v1/models';Auth='Bearer LMSTUDIO_KEY_M1_REDACTED'}, @{Name='M2';Url='http://192.168.1.26:1234/api/v1/models';Auth='Bearer LMSTUDIO_KEY_M2_REDACTED'}, @{Name='M3';Url='http://192.168.1.113:1234/api/v1/models';Auth='Bearer LMSTUDIO_KEY_M3_REDACTED'}, @{Name='OL1';Url='http://127.0.0.1:11434/api/tags';Auth=$null}); foreach($n in $nodes){$t = Measure-Command {try{if($n.Auth){Invoke-WebRequest -Uri $n.Url -Headers @{'Authorization'=$n.Auth} -TimeoutSec 5 -UseBasicParsing | Out-Null}else{Invoke-WebRequest -Uri $n.Url -TimeoutSec 5 -UseBasicParsing | Out-Null}; $ok=$true}catch{$ok=$false}}; $ms = [math]::Round($t.TotalMilliseconds); \"$($n.Name): $(if($ok){\"${ms}ms OK\"}else{'OFFLINE'})\"}"),
    ...
    ], "pipeline", "powershell:$db1 = Test-Path 'F:\\BUREAU\\turbo\\data\\etoile.db'; $db2 = Test-Path 'F:\\BUREAU\\turbo\\data\\jarvis.db'; \"Bases: etoile=$(if($db1){'OK'}else{'MANQUE'}) | jarvis=$(if($db2){'OK'}else{'MANQUE'})\";;powershell:$cmds = (Get-Content F:\\BUREAU\\turbo\\src\\commands.py -Raw | Select-String -Pattern 'JarvisCommand\\(' -AllMatches).Matches.Count; $pipes = (Get-Content F:\\BUREAU\\turbo\\src\\commands_pipelines.py -Raw | Select-String -Pattern 'JarvisCommand\\(' -AllMatches).Matches.Count; \"Commandes: $cmds | Pipelines: $pipes | Total: $($cmds + $pipes)\";;powershell:$m1 = try{(Invoke-WebRequest -Uri 'http://10.5.0.2:1234/api/v1/models' -Headers @{'Authorization'='Bearer LMSTUDIO_KEY_M1_REDACTED'} -TimeoutSec 3 -UseBasicParsing).StatusCode -eq 200}catch{$false}; $m2 = try{(Invoke-WebRequest -Uri 'http://192.168.1.26:1234/api/v1/models' -Headers @{'Authorization'='Bearer LMSTUDIO_KEY_M2_REDACTED'} -TimeoutSec 3 -UseBasicParsing).StatusCode -eq 200}catch{$false}; $ol1 = try{(Invoke-WebRequest -Uri 'http://127.0.0.1:11434/api/tags' -TimeoutSec 3 -UseBasicParsing).StatusCode -eq 200}catch{$false}; $online = @($m1,$m2,$ol1) | Where-Object {$_}; \"Cluster: $($online.Count)/3 noeuds en ligne | JARVIS operationnel\""),
    ...
    ], "pipeline", "powershell:\"=== TEST FAILOVER ===\"; \"1. Test M1 (principal)...\"; $m1 = try{$t=Measure-Command{Invoke-WebRequest -Uri 'http://10.5.0.2:1234/api/v1/models' -Headers @{'Authorization'='Bearer LMSTUDIO_KEY_M1_REDACTED'} -TimeoutSec 3 -UseBasicParsing | Out-Null}; \"OK ($([math]::Round($t.TotalMilliseconds))ms)\"}catch{\"OFFLINE\"}; \"   M1: $m1\";;powershell:\"2. Test M2 (fallback)...\"; $m2 = try{$t=Measure-Command{Invoke-WebRequest -Uri 'http://192.168.1.26:1234/api/v1/models' -Headers @{'Authorization'='Bearer LMSTUDIO_KEY_M2_REDACTED'} -TimeoutSec 3 -UseBasicParsing | Out-Null}; \"OK ($([math]::Round($t.TotalMilliseconds))ms)\"}catch{\"OFFLINE\"}; \"   M2: $m2\";;powershell:\"3. Test M3 (secondaire)...\"; $m3 = try{$t=Measure-Command{Invoke-WebRequest -Uri 'http://192.168.1.113:1234/api/v1/models' -Headers @{'Authorization'='Bearer LMSTUDIO_KEY_M3_REDACTED'} -TimeoutSec 3 -UseBasicParsing | Out-Null}; \"OK ($([math]::Round($t.TotalMilliseconds))ms)\"}catch{\"OFFLINE\"}; \"   M3: $m3\";;powershell:\"4. Test OL1 (rapide)...\"; $ol1 = try{$t=Measure-Command{Invoke-WebRequest -Uri 'http://127.0.0.1:11434/api/tags' -TimeoutSec 3 -UseBasicParsing | Out-Null}; \"OK ($([math]::Round($t.TotalMilliseconds))ms)\"}catch{\"OFFLINE\"}; \"   OL1: $ol1\"; \"=== Failover: M1->M2->M3->OL1 valide ===\""),
    ...
    ], "pipeline", "powershell:& 'C:\\Users\\franc\\.local\\bin\\uv.exe' run python -c \"import os,glob; path='F:/BUREAU/turbo/finetuning'; files=glob.glob(os.path.join(path,'**/*.jsonl'),recursive=True); cc=lambda f:sum(1 for _ in open(f,encoding='utf-8')); [print(f'  {os.path.basename(f)}: {cc(f)} examples ({round(os.path.getsize(f)/1024)}KB)') for f in files[:10]] if files else print('Aucun dataset JSONL trouve')\" 2>&1 | Out-String"),
    ...
    ], "pipeline", "powershell:Write-Output '=== INVENTAIRE MODELES ==='; try { $m1 = (Invoke-WebRequest -Uri 'http://10.5.0.2:1234/api/v1/models' -Headers @{Authorization='Bearer LMSTUDIO_KEY_M1_REDACTED'} -TimeoutSec 5 -UseBasicParsing).Content | ConvertFrom-Json; $m1Loaded = $m1.models | Where-Object { $_.loaded_instances.Count -gt 0 }; Write-Output \"  M1: $($m1Loaded.Count) charges [$($m1Loaded.key -join ', ')]\"; Write-Output \"  M1 total: $($m1.models.Count) disponibles\" } catch { Write-Output '  M1: offline' }; try { $m2 = (Invoke-WebRequest -Uri 'http://192.168.1.26:1234/api/v1/models' -Headers @{Authorization='Bearer LMSTUDIO_KEY_M2_REDACTED'} -TimeoutSec 5 -UseBasicParsing).Content | ConvertFrom-Json; $m2Loaded = $m2.models | Where-Object { $_.loaded_instances.Count -gt 0 }; Write-Output \"  M2: $($m2Loaded.Count) charges [$($m2Loaded.key -join ', ')]\" } catch { Write-Output '  M2: offline' }; try { $m3 = (Invoke-WebRequest -Uri 'http://192.168.1.113:1234/api/v1/models' -Headers @{Authorization='Bearer LMSTUDIO_KEY_M3_REDACTED'} -TimeoutSec 5 -UseBasicParsing).Content | ConvertFrom-Json; $m3Loaded = $m3.models | Where-Object { $_.loaded_instances.Count -gt 0 }; Write-Output \"  M3: $($m3Loaded.Count) charges [$($m3Loaded.key -join ', ')]\" } catch { Write-Output '  M3: offline' }; try { $ol1 = (Invoke-WebRequest -Uri 'http://127.0.0.1:11434/api/tags' -TimeoutSec 3 -UseBasicParsing).Content | ConvertFrom-Json; Write-Output \"  OL1: $($ol1.models.Count) modeles\" } catch { Write-Output '  OL1: offline' }"),
    ...
    ], "pipeline", "powershell:Write-Output '=== LATENCE API ==='; $targets = @(@{N='M1';U='http://10.5.0.2:1234/api/v1/models';H=@{Authorization='Bearer LMSTUDIO_KEY_M1_REDACTED'}},@{N='M2';U='http://192.168.1.26:1234/api/v1/models';H=@{Authorization='Bearer LMSTUDIO_KEY_M2_REDACTED'}},@{N='OL1';U='http://127.0.0.1:11434/api/tags';H=@{}}); foreach ($t in $targets) { try { $ms = (Measure-Command { Invoke-WebRequest -Uri $t.U -Headers $t.H -TimeoutSec 5 -UseBasicParsing }).TotalMilliseconds; Write-Output \"  $($t.N): $([math]::Round($ms))ms\" } catch { Write-Output \"  $($t.N): offline\" } }"),
    ...
    ], "pipeline", "powershell:Write-Output '=== PROFILS VOCAUX ==='; Write-Output '  Profil actuel: fr-FR-HenriNeural (homme)'; Write-Output '  Profils disponibles:'; Write-Output '    fr-FR-HenriNeural — Homme francais (defaut)'; Write-Output '    fr-FR-DeniseNeural — Femme francaise'; Write-Output '    fr-FR-EloiseNeural — Femme jeune'; Write-Output '    en-US-GuyNeural — Homme anglais'; Write-Output '  Modifier dans: src/voice.py (VOICE_NAME variable)'"),
    ...
    ], "pipeline", "powershell:$body = '{\"model\":\"qwen3-8b\",\"input\":\"/nothink\\nGenere des embeddings conceptuels pour les 5 requetes les plus frequentes de JARVIS: status cluster, diagnostic systeme, trading signal, routine matin, securite audit. Score pertinence /10.\",\"temperature\":0.2,\"max_output_tokens\":256,\"stream\":false,\"store\":false}'; try { $r = Invoke-WebRequest -Uri 'http://10.5.0.2:1234/api/v1/chat' -Method POST -Body $body -ContentType 'application/json' -Headers @{Authorization='Bearer LMSTUDIO_KEY_M1_REDACTED'} -TimeoutSec 20 -UseBasicParsing; $d = $r.Content | ConvertFrom-Json; $msg = ($d.output | Where-Object { $_.type -eq 'message' } | Select-Object -Last 1).content; Write-Output '=== PREWARM CACHE ==='; Write-Output $msg } catch { Write-Output 'Prewarm: M1 offline' }"),
    ...
    ], "pipeline", "powershell:$body = '{\"model\":\"qwen3-8b\",\"input\":\"/nothink\\nPrepare un contexte RAG pour la question: comment optimiser le cluster JARVIS? Genere les chunks pertinents: 1) architecture cluster 2) benchmark scores 3) thermal management. Format: chunk_id, contenu resume, score pertinence.\",\"temperature\":0.2,\"max_output_tokens\":256,\"stream\":false,\"store\":false}'; try { $r = Invoke-WebRequest -Uri 'http://10.5.0.2:1234/api/v1/chat' -Method POST -Body $body -ContentType 'application/json' -Headers @{Authorization='Bearer LMSTUDIO_KEY_M1_REDACTED'} -TimeoutSec 20 -UseBasicParsing; $d = $r.Content | ConvertFrom-Json; $msg = ($d.output | Where-Object { $_.type -eq 'message' } | Select-Object -Last 1).content; Write-Output $msg } catch { Write-Output 'RAG context: M1 offline' }"),
    ...
    ], "pipeline", "powershell:$body = '{\"model\":\"qwen3-8b\",\"input\":\"/nothink\\nAnalyse les performances du cluster JARVIS: M1(100%,0.6s), M2(90%,1.3s), M3(89%,2.5s), OL1(88%,0.5s). Poids actuels: M1=1.8, M2=1.4, OL1=1.3, M3=1.0. Propose des poids optimises bases sur performance*vitesse et justifie.\",\"temperature\":0.2,\"max_output_tokens\":256,\"stream\":false,\"store\":false}'; try { $r = Invoke-WebRequest -Uri 'http://10.5.0.2:1234/api/v1/chat' -Method POST -Body $body -ContentType 'application/json' -Headers @{Authorization='Bearer LMSTUDIO_KEY_M1_REDACTED'} -TimeoutSec 20 -UseBasicParsing; $d = $r.Content | ConvertFrom-Json; $msg = ($d.output | Where-Object { $_.type -eq 'message' } | Select-Object -Last 1).content; Write-Output '=== AUTO-TUNE POIDS ==='; Write-Output $msg } catch { Write-Output 'Auto-tune: M1 offline' }"),
    ...
    ], "pipeline", "powershell:$body = '{\"model\":\"qwen3-8b\",\"input\":\"/nothink\\nAnalyse VRAM: RTX2060 12GB (10.3GB used), GTX1660S 6GB (0.5GB used x4), M2 3x8GB, M3 1x8GB. Predit quand chaque GPU atteindra 95%% utilisation. Quelles GPU sont a risque dans les 4h?\",\"temperature\":0.2,\"max_output_tokens\":256,\"stream\":false,\"store\":false}'; try { $r = Invoke-WebRequest -Uri 'http://10.5.0.2:1234/api/v1/chat' -Method POST -Body $body -ContentType 'application/json' -Headers @{Authorization='Bearer LMSTUDIO_KEY_M1_REDACTED'} -TimeoutSec 20 -UseBasicParsing; $d = $r.Content | ConvertFrom-Json; $msg = ($d.output | Where-Object { $_.type -eq 'message' } | Select-Object -Last 1).content; Write-Output '=== PREDICTION VRAM ==='; Write-Output $msg } catch { Write-Output 'Prediction: M1 offline' }"),
