...
start "JARVIS-Evolution" /MIN python cowork/dev/strategy_evolution_loop.py --pop 1000 --coins 25 --interval 5
timeout /t 2 /nobreak >nul

echo [2/5] Autonomous Orchestrator v3 (30s cycles, dry-run)...
start "JARVIS-Orchestrator" /MIN python cowork/dev/autonomous_orchestrator_v3.py --interval 30 --dry
timeout /t 2 /nobreak >nul

echo [3/5] Cluster Strategy Worker (5min cycles)...
start "JARVIS-StrategyWorker" /MIN python cowork/dev/cluster_strategy_worker.py --interval 5
timeout /t 2 /nobreak >nul

echo [4/5] Cluster Deep Analysis Worker (5min cycles)...
start "JARVIS-DeepAnalysis" /MIN python cowork/dev/cluster_deep_analysis_worker.py --interval 5
timeout /t 2 /nobreak >nul

echo [5/5] MCP Server + Cloudflare Tunnel (Perplexity)...
start "JARVIS-MCP" /MIN python -m src.mcp_server_sse --port 8901 --full
