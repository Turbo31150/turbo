...
const CSS = `
@keyframes orbPulse{0%{box-shadow:0 0 0 0 ${COLORS.orangeAlpha(0.4)}}70%{box-shadow:0 0 0 25px ${COLORS.orangeAlpha(0)}}100%{box-shadow:0 0 0 0 ${COLORS.orangeAlpha(0)}}}
@keyframes orbGlow{0%,100%{box-shadow:0 0 20px ${COLORS.orangeAlpha(0.3)}}50%{box-shadow:0 0 40px ${COLORS.orangeAlpha(0.6)}}}
...
const S = {
  page: { display: 'flex', flexDirection: 'column', alignItems: 'center', padding: 24, fontFamily: FONT, height: '100%', overflow: 'hidden' } as React.CSSProperties,
  title: { fontSize: 18, fontWeight: 700, color: COLORS.text, marginBottom: 4 } as React.CSSProperties,
  subtitle: { fontSize: 11, color: COLORS.textDim, marginBottom: 20 } as React.CSSProperties,
  ...
  orb: { width: 100, height: 100, borderRadius: '50%', border: `3px solid ${COLORS.border}`, backgroundColor: COLORS.bgCard, display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', transition: 'all .3s', zIndex: 2, position: 'relative', flexDirection: 'column', gap: 4 } as React.CSSProperties,
  orbActive: { borderColor: COLORS.orange, animation: 'orbGlow 2s ease infinite' },
  ...
  ring: { position: 'absolute', width: 100, height: 100, borderRadius: '50%', border: `2px solid ${COLORS.orange}`, animation: 'ringExpand 1.5s ease infinite', zIndex: 1 } as React.CSSProperties,
  bars: { display: 'flex', alignItems: 'center', gap: 3, height: 36 } as React.CSSProperties,
  bar: { width: 4, backgroundColor: COLORS.orange, borderRadius: 2, transition: 'height .1s' } as React.CSSProperties,
  controls: { display: 'flex', gap: 8 } as React.CSSProperties,
  toggle: { padding: '4px 10px', borderRadius: 6, border: `1px solid ${COLORS.border}`, backgroundColor: 'transparent', color: COLORS.textDim, fontSize: 10, cursor: 'pointer', fontFamily: 'inherit', transition: 'all .2s', textTransform: 'uppercase', letterSpacing: 1 } as React.CSSProperties,
  toggleOn: { borderColor: COLORS.green, color: COLORS.green, backgroundColor: COLORS.greenAlpha(0.08) },
  inputWrap: { display: 'flex', gap: 8, width: '100%', maxWidth: 650, marginBottom: 12 } as React.CSSProperties,
  textInput: { flex: 1, backgroundColor: COLORS.bg, border: `1px solid ${COLORS.border}`, borderRadius: 8, color: COLORS.text, fontFamily: 'inherit', fontSize: 13, padding: '10px 14px', outline: 'none' } as React.CSSProperties,
  sendBtn: { padding: '0 16px', background: `linear-gradient(135deg,${COLORS.orange},${COLORS.orangeDark})`, color: '#fff', border: 'none', borderRadius: 8, fontWeight: 700, fontFamily: 'inherit', fontSize: 12, cursor: 'pointer', letterSpacing: 1 } as React.CSSProperties,
  ...
  msgUser: { alignSelf: 'flex-end', maxWidth: '80%', padding: '8px 14px', borderRadius: '12px 12px 2px 12px', backgroundColor: COLORS.border, border: `1px solid ${COLORS.border}`, fontSize: 13, color: COLORS.text, lineHeight: 1.5 } as React.CSSProperties,
  msgAssistant: { alignSelf: 'flex-start', maxWidth: '80%', padding: '8px 14px', borderRadius: '12px 12px 12px 2px', backgroundColor: COLORS.bgCard, border: `1px solid ${COLORS.orange}`, fontSize: 13, color: COLORS.text, lineHeight: 1.5 } as React.CSSProperties,
  msgSystem: { alignSelf: 'center', padding: '4px 12px', borderRadius: 6, backgroundColor: COLORS.bgDarker, fontSize: 11, color: COLORS.textDim } as React.CSSProperties,
  agentTag: { display: 'inline-block', fontSize: 9, color: COLORS.orange, backgroundColor: COLORS.orangeAlpha(0.1), padding: '1px 6px', borderRadius: 4, marginBottom: 4, letterSpacing: .5, textTransform: 'uppercase' } as React.CSSProperties,
  time: { fontSize: 9, color: COLORS.textDimmer, marginTop: 4, textAlign: 'right' } as React.CSSProperties,
  thinking: { alignSelf: 'flex-start', display: 'flex', alignItems: 'center', gap: 6, padding: '8px 14px', borderRadius: '12px 12px 12px 2px', backgroundColor: COLORS.bgCard, border: `1px solid ${COLORS.border}`, fontSize: 12, color: COLORS.textDim } as React.CSSProperties,
  dot: { width: 6, height: 6, borderRadius: '50%', backgroundColor: COLORS.orange } as React.CSSProperties,
  statusBar: { display: 'flex', justifyContent: 'space-between', width: '100%', maxWidth: 650, fontSize: 10, color: COLORS.textDimmer, marginBottom: 8 } as React.CSSProperties,
};
    ...
    <>
      ...
      <div style={S.page}>
        ...
        {transcriptions.length > 0 && (
          <div style={{ width: '100%', maxWidth: 650, marginBottom: 8, padding: '6px 12px', borderRadius: 6, backgroundColor: COLORS.purpleAlpha(0.06), border: `1px solid ${COLORS.purpleAlpha(0.15)}`, fontSize: 11, color: COLORS.purple, display: 'flex', gap: 8, alignItems: 'center' }}>
            ...
            <span style={{ fontSize: 9, color: COLORS.textDimmer }}>
        ...
        <div style={S.convWrap}>
          {conversation.length === 0 && !thinking ? (
            <div style={{ textAlign: 'center', color: COLORS.textDimmer, fontSize: 12, padding: 40 }}>
