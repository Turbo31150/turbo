"""Test WebSocket: message simple."""
import asyncio, json, time, websockets

async def test():
    uri = "ws://127.0.0.1:9742/ws"
    async with websockets.connect(uri) as ws:
        print("=== TEST: Message simple via WebSocket ===")
        envelope = {
            "id": "test_simple_001",
            "type": "request",
            "channel": "chat",
            "action": "send_message",
            "payload": {"content": "Explique-moi ce qu'est un transformer en IA en 3 phrases"},
        }
        t0 = time.time()
        ...
        while True:
            try:
                ...
                if msg.get("type") == "event" and msg.get("event") == "agent_complete":
                    break
                if msg.get("type") == "response" and msg.get("id") == "test_simple_001":
                    try:
                        while True:
        ...
        for r in responses:
            if r.get("type") == "response" and r.get("channel") == "chat":
                ...
                content = am.get("content", "")[:300]
                print(f"CONTENT: {content}")
                if r.get("error"):
                    print(f"ERROR:   {r['error']}")
            elif r.get("type") == "event":
                evt = r.get("event", "?")
                pl = json.dumps(r.get("payload", {}), ensure_ascii=False)[:150]
